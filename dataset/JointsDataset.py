...
class JointsDataset(Dataset):
    ...
    def __init__(self, DATASET, stage, transform=None):
        ...
        self.num_keypoints_half_body = DATASET.TRAIN.NUM_KEYPOINTS_HALF_BODY  # what ???
    ...
    def __getitem__(self, idx):
        ...
        data_numpy = cv.imread(img_path, cv.IMREAD_COLOR)

        if data_numpy is None:
            raise ValueError('fail to read {}'.format(img_path))

        if self.color_rgb:
            data_numpy = cv.cvtColor(data_numpy, cv.COLOR_BGR2RGB)

        joints = d['joints'][:, :2]
        # 0: Not marked; 1: Not visible; 2: Normal point
        ...
        if self.stage == 'train':
            # x:（1 + 0.05） * （1 ~1.6） = (1.05~1.68)
            # y:（1 + 0.05） * （1 ~1.8） = (1.05~1.89)
        ...
        if self.stage == 'train':
            for i in range(self.keypoint_num):
                if joints_vis[i, 0] > 0:
                    ...
                    # Label 0 when out of image range.
                    if joints[i, 0] < 0 or joints[i, 0] > self.input_shape[1] - 1 or joints[i, 1] < 0 \
    ...
    def half_body_transform(self, joints, joints_vis):
        ...
        # 2: My understanding is that there are at least the upper left corner and the lower right corner.
        if np.random.randn() < 0.5 and len(upper_joints) > 2:
            selected_joints = upper_joints
        else:
            selected_joints = lower_joints if len(lower_joints) > 2 \
                else upper_joints
    ...
    def generate_heatmap(self, joints, valid, kernel=(7, 7)):
        ...
        for i in range(self.keypoint_num):
            ...
            heatmaps[i] = cv.GaussianBlur(heatmaps[i], kernel, 0)
