...
def compute_on_dataset(model, data_loader, device):
    ...
    for _, batch in enumerate(data):
        ...
        for i in range(preds.shape[0]):
            ...
            score = scores[i] * kp_scores[i]  # Combine box scores
            image_id = img_ids[i]
...
def main():
    ...
    # output: image_id, category_id, keypoints, score
    ...
    if is_main_process():
        ...
        results.sort(
            key=lambda res: (res['image_id'], res['score']), reverse=True)  # Descending
...
if __name__ == '__main__':
    ...
    from dataset.MDJPE.mdj import MDJDataset
    ...
    for i in range(len(mdj)):
        annotations['image_ids'].add(data[i]['img_id'])
        annotations['annos'][data[i]['img_id']] = dict()
        annotations['annos'][data[i]['img_id']]['human_annos'] = data[i]['joints']
        annotations['annos'][data[i]['img_id']]['keypoint_annos'] = data[i]['bbox']
